...
public struct CharacterSpawnRequest : IComponentData {
    ...
    public static void Create(EntityCommandBuffer commandBuffer, int characterType, Vector3 position,
        Quaternion rotation, Entity playerEntity) {
...
public class HandleCharacterSpawnRequests : BaseComponentSystem {
    ...
    public HandleCharacterSpawnRequests(GameWorld world, BundledResourceManager resourceManager, bool isServer) :
        base(world) {
    ...
    protected override void OnUpdate() {
        ...
        for (var i = 0; i < spawnRequests.Length; i++) {
            ...
            var character = SpawnCharacter(m_world, playerState, request.position, request.rotation,
                request.characterType, m_ResourceManager);
...
//            character.item = heroTypeAsset.items[0].weaponType.Create(EntityManager, owner.playerId);
//            var itemState = EntityManager.GetComponentData<WeaponTypeDefinition.State>(character.item);
...
//        ItemGroup = GetComponentGroup(typeof(CharacterItem));
    ...
    protected override void OnUpdate() {
        ...
        for (var i = 0; i < requestArray.Length; i++) {
            ...
            Debug.Log(request.characterEntity);
            ...
            if (character.item != Entity.Null)
                m_world.RequestDespawn(PostUpdateCommands, character.item);


            PostUpdateCommands.DestroyEntity(requestEntityArray[i]);
            GameDebug.Log("Character despawned:" + character.name + " tick:" + m_world.worldTime.tick);
        }
...
public class HandleDamage : BaseComponentSystem<HealthState, HitCollisionOwner> {
    ...
    protected override void Update(Entity entity, HealthState healthState, HitCollisionOwner hitCollisionOwner) {
        ...
        for (var eventIndex = 0; eventIndex < damageEvents.Count; eventIndex++) {
            ...
            if (damageEvents[eventIndex].instigator != Entity.Null && EntityManager.Exists(damageEvent.instigator) &&
                EntityManager.HasComponent<DamageHistory>(damageEvent.instigator)) {
...
public class CharacterModuleServer : CharacterModuleShared {
    public CharacterModuleServer(GameWorld world, BundledResourceManager resourceSystem) : base(world) {
        ...
        m_HandleCharacterSpawnRequests = m_world.GetECSWorld()
            .CreateManager<HandleCharacterSpawnRequests>(m_world, resourceSystem, true);
        ...
        m_ControlledEntityChangedSystems.Add(m_world.GetECSWorld()
            .CreateManager<PlayerCharacterControlSystem>(m_world));
        ...
        m_UpdatePresentationRootTransform =
            m_world.GetECSWorld().CreateManager<UpdatePresentationRootTransform>(m_world);
        m_UpdatePresentationAttachmentTransform =
            m_world.GetECSWorld().CreateManager<UpdatePresentationAttachmentTransform>(m_world);
    }
