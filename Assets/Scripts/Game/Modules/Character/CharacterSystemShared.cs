...
public class HandleCharacterSpawn : InitializeComponentGroupSystem<Character, HandleCharacterSpawn.Initialized> {
    ...
    public HandleCharacterSpawn(GameWorld gameWorld, BundledResourceManager resourceManager, bool server) :
        base(gameWorld) {
    ...
    protected override void Initialize(ref ComponentGroup group) {
        ...
        for (var i = 0; i < entityBuffer.Count; i++) {
            ...
            // Setup abilities
            GameDebug.Assert(EntityManager.Exists(character.behaviourController),
                "behavior controller entity does not exist");
            GameDebug.Assert(EntityManager.HasComponent<CharBehaviour>(character.behaviourController),
                "behavior controller entity does not have CharBehaviour component");
    ...
    protected override void Update(Entity entity, Character character) {
        if (character.m_TeleportPending) {
            ...
            predictedState.velocity =
                character.m_TeleportToRotation * Vector3.forward * predictedState.velocity.magnitude;
...
public class UpdateCharPresentationState : BaseComponentSystem {
    ...
    protected override void OnUpdate() {
        ...
        for (var i = 0; i < charPredictedStateArray.Length; i++) {
            ...
            animState.rightControllerPos = userCommand.rightControllerPos;
            animState.leftControllerPos = userCommand.leftControllerPos;
            animState.headsetPos = userCommand.headsetPos;
...
public class GroundTest : BaseComponentSystem {
    ...
    protected override void OnUpdate() {
        ...
        for (var i = 0; i < characterArray.Length; i++) {
            ...
            character.altitude = character.groundCollider != null
                ? rayResults[i].distance - startOffset
...
public class ApplyPresentationState : BaseComponentSystem {
    ...
    protected override void OnCreateManager() {
        base.OnCreateManager();
        CharGroup = GetComponentGroup(typeof(AnimStateController), typeof(CharPresentation),
