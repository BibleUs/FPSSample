...
[DisableAutoCreation]
class AutoRifle_RequestActive : BaseComponentDataSystem<CharBehaviour, AbilityControl, Ability_AutoRifle.PredictedState,
    Ability_AutoRifle.Settings> {
    ...
    protected override void Update(Entity entity, CharBehaviour charAbility, AbilityControl abilityCtrl,
        ...
        abilityCtrl.behaviorState = request != Ability_AutoRifle.Action.Idle
            ? AbilityControl.State.RequestActive
            : AbilityControl.State.Idle;
        EntityManager.SetComponentData(entity, abilityCtrl);
    }
}

[DisableAutoCreation]
class AutoRifle_Update : BaseComponentDataSystem<CharBehaviour, AbilityControl, Ability_AutoRifle.PredictedState,
    Ability_AutoRifle.Settings> {
...
[DisableAutoCreation]
class AutoRifle_HandleCollisionQuery : BaseComponentDataSystem<Ability_AutoRifle.InternalState,
    Ability_AutoRifle.InterpolatedState> {
    public AutoRifle_HandleCollisionQuery(GameWorld world) : base(world) {
        ExtraComponentRequirements = new ComponentType[] {typeof(ServerEntity)};
    }

    protected override void Update(Entity abilityEntity, Ability_AutoRifle.InternalState internalState,
        Ability_AutoRifle.InterpolatedState interpolatedState) {
        ...
        if (impact) {
            ...
            if (hitCollisionHit) {
                ...
                hitCollisionOwner.damageEvents.Add(new DamageEvent(charAbility.character, settings.damage,
                    query.direction,
